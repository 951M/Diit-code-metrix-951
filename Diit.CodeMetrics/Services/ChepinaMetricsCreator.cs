...
namespace Diit.CodeMetrics.Services
{
    public class ChepinaMetricsCreator : IMetricsCreator<IChepinaMetrics>
    {
        ...
        public IChepinaMetrics CreateMetrics(List<Data.Source.Module> source)
        {
            ...
            metrics.ChMetrics = Calculate(analized.CommentCounter, analized.OperatorsCounter, analized.LineNumber,
            ...
            List<int> operatorsBlockCounter, List<int> commentBlockCounter, bool isprime, double a_coef)
        {
            ...
            try
            {
                ...
                metriks.TryAdd("Количество комментариев111", Ncomments);
                metriks.TryAdd("Количество блоков111", commentBlockCounter.Count);
                ...
                metriks.TryAdd("Комментированность1111 общая через строки (" + analysLines + ")", Math.Round(f1, 4));

                analysLines = "Недостаточно";
                if (f2 > 0.1f)
                    analysLines = "Достаточно";

                metriks.TryAdd("Комментированность111 общая через операторы (" + analysLines + ")", Math.Round(f2, 4));
